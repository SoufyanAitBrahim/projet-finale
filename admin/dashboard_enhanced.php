...
// Get grouped orders count (count unique orders by client and time window)
...
// Group orders by client and time window (same logic as orders_management.php)
$grouped_orders = [];
foreach ($orders_raw as $order) {
    ...
    if (!isset($grouped_orders[$group_key])) {
        $grouped_orders[$group_key] = true; // Just mark as existing
    }
}
$orders_count = count($grouped_orders);
...
// Recent menu orders (grouped orders from ORDER table)
$recent_orders_raw = $pdo->query("
    SELECT
        o.*,
...
// Group recent orders (same logic as orders_management.php)
$grouped_recent_orders = [];
foreach ($recent_orders_raw as $order) {
    $order_time = strtotime($order['DATE_ORDER']);
    $hour_key = floor($order_time / 3600); // Group by hour
    $group_key = $order['ID_CLIENTS'] . '_' . $hour_key . '_' . $order['ORDER_SITUATION'] . '_' . $order['PAYMENT_SITUATION'];

    if (!isset($grouped_recent_orders[$group_key])) {
        $grouped_recent_orders[$group_key] = [
            'client_info' => [
    ...
    $grouped_recent_orders[$group_key]['meals'][] = [
        'meal_name' => $order['meal_name'],
        'meal_price' => $order['meal_price'],
        'category_name' => $order['category_name']
    ];

    $grouped_recent_orders[$group_key]['total_price'] += $order['meal_price'];
    $grouped_recent_orders[$group_key]['meal_count']++;
}

// Convert to indexed array and sort by date, limit to 5
$recent_menu_orders = array_values($grouped_recent_orders);
usort($recent_menu_orders, function($a, $b) {
    return strtotime($b['order_info']['DATE_ORDER']) - strtotime($a['order_info']['DATE_ORDER']);
});
$recent_menu_orders = array_slice($recent_menu_orders, 0, 5);
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <small><?php echo date('M d', strtotime($order['order_info']['DATE_ORDER'])); ?></small><br>
                                            <small class="text-muted"><?php echo date('H:i', strtotime($order['order_info']['DATE_ORDER'])); ?></small>
                                        </td>
                                        <td>
                                            <strong><?php echo htmlspecialchars($order['client_info']['client_name']); ?></strong><br>
                                            <small class="text-muted"><?php echo htmlspecialchars($order['client_info']['client_phone']); ?></small>
                                        </td>
                                        <td>
                                            <span class="badge bg-primary"><?php echo $order['meal_count']; ?> item(s)</span><br>
                                            <small class="text-muted">
                                                <?php
                                                $meal_names = array_column($order['meals'], 'meal_name');
                                                $display_meals = implode(', ', array_slice($meal_names, 0, 2));
                                                if (count($meal_names) > 2) {
                                                    $display_meals .= '...';
                                                }
                                                echo htmlspecialchars($display_meals);
                                                ?>
                                            </small>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span class="badge bg-<?php
                                                echo $order['order_info']['ORDER_SITUATION'] == 'Pending' ? 'warning' :
                                                    ($order['order_info']['ORDER_SITUATION'] == 'Confirmed' ? 'info' :
                                                    ($order['order_info']['ORDER_SITUATION'] == 'Preparing' ? 'primary' :
                                                    ($order['order_info']['ORDER_SITUATION'] == 'Ready' ? 'success' :
                                                    ($order['order_info']['ORDER_SITUATION'] == 'Delivered' ? 'secondary' :
                                                    ($order['order_info']['ORDER_SITUATION'] == 'Cancelled' ? 'danger' : 'dark')))));
                                            ?>">
                                                <?php echo htmlspecialchars($order['order_info']['ORDER_SITUATION']); ?>
                                            </span><br>
                                            <small class="badge bg-<?php echo $order['order_info']['PAYMENT_SITUATION'] == 1 ? 'success' : ($order['order_info']['PAYMENT_SITUATION'] == 2 ? 'warning' : 'secondary'); ?>">
                                                <?php
                                                echo $order['order_info']['PAYMENT_SITUATION'] == 0 ? 'Payment Pending' :
                                                    ($order['order_info']['PAYMENT_SITUATION'] == 1 ? 'Paid' :
                                                    ($order['order_info']['PAYMENT_SITUATION'] == 2 ? 'Refunded' : 'Unknown'));
                                                ?>
                                            </small>
                                        </td>
                                    </tr>
                        ...
                        <div class="text-center py-4">
                            ...
                            <p class="text-muted">No recent menu orders</p>
                            <small class="text-muted">Orders will appear here when customers place them</small>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
